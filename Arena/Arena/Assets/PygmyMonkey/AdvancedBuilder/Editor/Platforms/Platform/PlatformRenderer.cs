...
namespace PygmyMonkey.AdvancedBuilder
{
	public class PlatformRenderer : IDefaultRenderer
	{
		 ...
		 */
		public void drawBuildSummary(AdvancedSettings advancedSettings, Configuration configuration, ProductParameters productParameters)
		{
			...
			GUIUtils.DrawTwoColumns("Bundle version", productParameters.bundleVersion);
			GUIUtils.DrawTwoColumns("Build number", productParameters.buildNumber + " (will increment if multiple builds)");
			...
			if (m_platformAdditionalRenderer != null)
			{
				m_platformAdditionalRenderer.drawAdditionalBuildSummary(configuration.platformArchitecture, configuration.textureCompression, productParameters);
			}

			EditorGUILayout.LabelField(configuration.getBuildPath(advancedSettings, DateTime.Now, productParameters));
		}
	}
}