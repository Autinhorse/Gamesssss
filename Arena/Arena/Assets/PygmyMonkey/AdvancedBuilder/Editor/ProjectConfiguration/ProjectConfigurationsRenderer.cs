	...
	public class ProjectConfigurationsRenderer : IDefaultRenderer
	{
					...
					GUILayout.BeginVertical();
					{
						...
						if (GUIUtils.DrawHeader("Build information"))
						{
							GUIUtils.BeginContents();
							{
								...
								platformRenderer.drawBuildSummary(m_advancedBuilder.getAdvancedSettings(), configuration, m_advancedBuilder.getProductParameters());
							}
		...
		public void checkWarningsAndErrors(ErrorReporter errorReporter)
		{
			...
			else
			{
				...
				if (m_projectConfigurations.configurationList.GroupBy(x => x.getBuildPath(m_advancedBuilder.getAdvancedSettings(), System.DateTime.Now, m_advancedBuilder.getProductParameters())).SelectMany(group => group.Skip(1)).Count() != 0)
				{
				...
				foreach (Configuration configuration in m_projectConfigurations.configurationList)
				{
					string finalBuildPath = Application.dataPath.Replace("/Assets", string.Empty) + configuration.getBuildPath(m_advancedBuilder.getAdvancedSettings(), System.DateTime.Now, m_advancedBuilder.getProductParameters());
